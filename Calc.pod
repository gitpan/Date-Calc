...
=head1 SYNOPSIS
  ...
  Easter_Orthodox_Sunday
      ($year,$month,$day) = Easter_Orthdox_Sunday($year [, $julian]);
...
C<($year,$month,$day) = Easter_Orthdox_Sunday($year [, $julian]);>

This function calculates the date of Easter Sunday for all years in the
range from 326 to 2299 (all other year numbers will result in a fatal
"year out of range" error message) using the method known as the "Gaussian
Rule".The problem is that the eastern (greek) orthodox church, uses the
Julian Calendar way of calculating Easter, but then applies +13 days
of Gregorian Calendar offset.
...
This man page documents "Date::Calc" version 5.5.
