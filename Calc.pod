...
=head1 SYNOPSIS
  ...
  Easter_Sunday
      ($year,$month,$day) = Easter_Sunday($year [, $orthodox [, $julian]]);
...
be used for years in the range 1583 to 2299 for Gregorian 
Calculations and 326 to 2299 for (revised) Julian (ie 
Orthodox) calculations.
...
barretclark notes that ``This actually happened with the Emperor Constantin,
who converted to Christianity but still worshipped the Sun god. He moved 
the Christian sabbath to the day of the Sun.'' 
...
See also L<perlfunc/gmtime>. Optional argument time (seconds since the epoch)
will of course return ($year,$month,$day, $hour,$min,$sec, $doy,$dow,$dst) 
for that particular time (thanks to Qiang for noting this).
...
C<($year,$month,$day) = Easter_Sunday($year [, $orthodox [, $julian]]);>

This function calculates the date of Easter Sunday for all years in the
range from 1583 to 2299 for Gregorian - default - and 326 to 2299 for 
(revised) Julian (ie Orthodox) (all other year numbers will result in a fatal
"year out of range" error message) using the method known as the "Gaussian
Rule".
The switch $orthodox  uses the Julian Calendar way of calculating Easter, 
but then applies +13 days of revised Julian Calendar offset.
Note that asking for Easter_Orthdox_Sunday with $year < 1923 or setting 
$julian to a true value will return the original Julian Date of Easter, 
since the Gregorian (actually revised Julian) calendar wasn't adopted 
in Greece before 1923 and still isn't (probably will never be) in some 
orthodox countries - notably Russia, Serbia and Georgia according to 
Wikipedia.
...
This man page documents "Date::Calc" version 5.5.1.
